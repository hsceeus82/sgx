...
class Crawl
{
    ...
    public $url;
    public $jar="";
    public $agent="Mozilla/5.0 (Windows NT 6.1; rv:24.0) Gecko/20100101 Firefox/24.0";
    public $dom;
    public $proxy_ip;
    ...
    public function curl($fields=array(),$post=true){
        $this->curl_init();
        if($post){
		...
		//start set proxy here
		/*
		$arrProxy = self::random_proxy();
		$loginpassw = 'ssoon:fgP56R31';  //your proxy login and password here
		$proxy_ip = $arrProxy['ip']; //proxy IP here
		$this->proxy_ip = $proxy_ip;
		$proxy_port = $arrProxy['port']; //proxy port from your proxy list
		$proxy_type = $arrProxy['type'];
		...
		*/
        ...
        curl_setopt($this->ch, CURLOPT_USERAGENT,  self::random_user_agent());
        ...
        if($this->jar!=""){
            ...
            curl_setopt($this->ch, CURLOPT_COOKIEFILE, $this->jar); //saved cookies
        }
    ...
    public function parse($content,$pattern=array(),$options=array()){
        $node=$this->dom->str_get_html($content);
	        
        foreach($pattern as $p){
            $index=0;
            if(is_array($p)){
                if(array_key_exists('idx', $p)){
                    $index=$p['idx'];
                }
                $node=$node->find($p[0],$index);
            }else{
                $node=$node->find($p,$index);
            }
        }
        ...
        foreach($options as $o){
            if(is_array($o)){
                 if(array_key_exists('REPLACE', $p)){
                    $result= preg_replace($p['REPLACE'][0], $p['REPLACE'][1], $result);
                }               
            }else{
                switch($o){
                    case "INT":
                        $result=get_int($result);
                        break;
                    case "STRIP":
	...
	{
		// 50.118.140.78 // backup proxy IP
		$arrIp = array('136.0.119.15','136.0.119.193','136.0.119.230','50.118.140.199');
		$arrPort = array('29842','29842','29842','29842');
		$arrType = array('HTTP','HTTP','HTTP','HTTP');
						
		$iTotalList = count($arrIp);
		$iArrayKey = rand(0, ($iTotalList-1));
		
		$arrResult['ip'] = $arrIp[$iArrayKey];
		$arrResult['port'] = $arrPort[$iArrayKey];
		$arrResult['type'] = $arrType[$iArrayKey];
